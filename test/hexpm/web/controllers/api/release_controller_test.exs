defmodule Hexpm.API.ReleaseControllerTest do
  ...
  test "create release", c do
    ...
    user_id = c.user.id
    package = assert Hexpm.Repo.get_by(Package, name: "ecto")
    assert package.name == "ecto"
    assert [%User{id: ^user_id}] = assoc(package, :owners) |> Hexpm.Repo.all
  ...
  test "update package", c do
    ...
    assert Hexpm.Repo.get_by(Package, name: c.package.name).meta.description == "awesomeness"
  end
  ...
  test "create releases", c do
    ...
    assert conn.status == 201
    postgrex = Hexpm.Repo.get_by!(Package, name: "postgrex")
    ...
    assert [%Release{package_id: ^postgrex_id, version: %Version{major: 0, minor: 0, patch: 2}},
           ...
           Release.all(postgrex) |> Hexpm.Repo.all |> Release.sort

    Hexpm.Repo.get_by!(assoc(postgrex, :releases), version: "0.0.1")
  end

  test "create release also creates package", c do
    refute Hexpm.Repo.get_by(Package, name: "phoenix")
    ...
    assert conn.status == 201
    assert %Package{name: "phoenix"} = Hexpm.Repo.get_by(Package, name: "phoenix")
  end
  ...
  test "delete release", c do
    ...
    assert conn.status == 201
    package = Hexpm.Repo.get_by!(Package, name: "postgrex")
    release = Hexpm.Repo.get_by!(assoc(package, :releases), version: "0.0.1")
    Ecto.Changeset.change(release, inserted_at: %{NaiveDateTime.utc_now | year: 2000})
    |> Hexpm.Repo.update!
    ...
    refute Hexpm.Repo.get_by(Package, name: "postgrex")
    refute Hexpm.Repo.get_by(assoc(package, :releases), version: "0.0.1")

    [_, log] = Hexpm.Repo.all(AuditLog)
  ...
  test "create releases with requirements", c do
    ...
    release = Hexpm.Repo.get_by(Package, name: "postgrex")
              |> assoc(:releases)
