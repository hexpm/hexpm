defmodule Hexpm.Docs.FilesTest do
  ...
  test "kinds/0 returns the kinds in canonical order" do
    assert Files.kinds() ==
             [:readme, :changelog, :license, :security, :threat_model, :support, :acknowledgments]
  end

  test "resolve_all/1 recognizes threat model and acknowledgments files" do
    assert Files.resolve_all(["THREAT_MODEL.md", "acknowledgements.txt"]) == %{
             threat_model: "THREAT_MODEL.md",
             acknowledgments: "acknowledgements.txt"
           }

    assert Files.resolve_all(["threatmodel.md"]) == %{threat_model: "threatmodel.md"}
    assert Files.resolve_all(["THREAT-MODEL"]) == %{threat_model: "THREAT-MODEL"}
  end
