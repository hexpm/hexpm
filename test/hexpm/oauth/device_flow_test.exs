defmodule Hexpm.OAuth.DeviceFlowTest do
  ...
  alias Hexpm.OAuth.{DeviceFlow, DeviceCode, Token}

  describe "initiate_device_authorization/2" do
    test "creates device code with default parameters" do
      assert {:ok, response} = DeviceFlow.initiate_device_authorization("test_client", ["api"])
      ...
      assert device_code
      assert device_code.client_id == "test_client"
    ...
    test "creates device code with custom scopes" do
      ...
      assert {:ok, response} = DeviceFlow.initiate_device_authorization("test_client", scopes)
    ...
    test "generates unique codes" do
      {:ok, response1} = DeviceFlow.initiate_device_authorization("client1", ["api"])
      {:ok, response2} = DeviceFlow.initiate_device_authorization("client2", ["api"])
  ...
  describe "poll_device_token/2" do
    ...
    test "returns authorization_pending for pending device code", %{response: response} do
      assert {:error, :authorization_pending, "Authorization pending"} =
               DeviceFlow.poll_device_token(response.device_code, "test_client")
    end

    test "returns invalid_grant for non-existent device code" do
      assert {:error, :invalid_grant, "Invalid device code"} =
               DeviceFlow.poll_device_token("nonexistent", "test_client")
    end
    ...
    test "returns expired_token for expired device code", %{device_code: device_code} do
      ...
      assert {:error, :expired_token, "Device code has expired"} =
               DeviceFlow.poll_device_token(device_code.device_code, "test_client")
    ...
    test "returns access_denied for denied device code", %{device_code: device_code} do
  ...
  describe "authorize_device/2" do
    ...
    test "successfully authorizes device", %{device_code: device_code, user: user} do
  ...
  describe "cleanup_expired_device_codes/0" do
    test "marks expired pending device codes as expired" do
      ...
      {:ok, _response2} = DeviceFlow.initiate_device_authorization("test_client2", ["api"])
