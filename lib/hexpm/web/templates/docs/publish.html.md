...
Before publishing documentation, Hex will build the documentation by running the `mix docs` task to make sure it is up to date. The main documentation tool for Elixir, `ex_doc` provides this task so if you add it as a dependency you don't have to do anything else to get automatic documentation builds when you publish your package. Check out the (documentation for ex_doc)[https://hexdocs.pm/ex_doc/readme.html] for information on how to configure your docs, we recommend building your docs locally with `mix docs` before publishing them to Hex. Finally also take a look out the (Elixir guide for writing documentation)[https://hexdocs.pm/elixir/writing-documentation.html] for suggestions and best practices.
