defmodule Hexpm.OAuth.Tokens do
  ...
  defp build_token(principal, client_id, scopes, grant_type, grant_reference, opts) do
    ...
    authorized = authorized_scopes(principal, scopes)
    ...
    |> maybe_add_refresh_token(principal, scopes, opts)
    |> Token.build()
  end

  # A user reaches the organizations they are currently a member of, after the
  # literal `repositories` scope has been expanded into names. An organization
  # subject reaches only itself.
  defp authorized_scopes(%User{} = user, scopes) do
    user
    |> Permissions.expand_repositories_scope(scopes)
    |> Permissions.reject_unaffiliated_scopes(user)
  end

  defp authorized_scopes(%Organization{} = org, scopes) do
