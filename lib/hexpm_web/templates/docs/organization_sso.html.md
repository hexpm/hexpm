...
* Organization membership for every person who will link an SSO identity, unless [just-in-time membership](#just-in-time-membership) or [provisioning](#provisioning-scim) will add them.
...
The application must allow the `openid` and `email` scopes. Hexpm uses the provider subject as the stable identity. The email claim never proves ownership of a Hexpm account and never selects which account a login links to. Hexpm shows it on the link confirmation screen, emails a member when a later login reports an address that isn't verified on their account, lets [provisioning](#provisioning-scim) match a current member by it, and, when [just-in-time membership](#just-in-time-membership) is on, admits a person only if it's on a verified domain and the provider marks it as verified.
...
Just-in-time membership admits people by the domain of the address your provider reports, so the organization first proves it controls that domain. On the organization's **SSO** dashboard, under **Verified domains**, add the domain, publish the TXT record shown there on the domain itself, and select **Verify**. The record value starts with `hexpm-domain-verification=`. If the record isn't found yet, wait for the DNS change to propagate and verify again.

Only exact matches count. Verifying `example.com` doesn't cover `eng.example.com`, so add and verify each domain your provider's addresses use.

Verification isn't permanent. A daily job at 03:15 UTC looks up the record of every verified domain that hasn't been checked in the last 24 hours, and clears the verification when the record is gone. Keep the record published for as long as you use just-in-time membership. A lookup that gets no answer, such as a DNS timeout, leaves the verification as it is.

Adding, verifying, and removing a domain take an organization administrator and step-up re-authentication, and each is recorded in the organization's audit log. A verification the daily job clears is recorded there too.

### Just-in-time membership

Just-in-time membership adds an existing Hexpm account to the organization the first time it authenticates through your provider, so you don't have to add each person before they link. It never creates a Hexpm account and never sends an invitation. Someone without a Hexpm account signs up before their first login, or you use [provisioning](#provisioning-scim), which can invite them.

It needs a saved SSO configuration and at least one [verified domain](#verified-domains). Turn it on under **Just-in-time membership** on the **SSO** dashboard by choosing what happens **When the seats run out**, and pick the **Role for new members**, **Read** or **Write**. Choosing **Off, do not add members automatically** turns it off again.
...
The person sees the same confirmation screen as any other link, and joins with the role you chose when they confirm it. Abandoning that screen adds nothing. The join is recorded in the organization's audit log as a member being added. A login whose address isn't on a verified domain is refused as not a member, the same answer an organization without just-in-time membership gives, so the refusal doesn't reveal whether the organization uses it or which domains it has verified.

While just-in-time membership is on, removing a member in Hexpm doesn't keep them out: their next login through your provider admits them again. Remove their assignment in your provider as well.
...
Hexpm refuses to switch an organization to required unless at least one administrator is exempt or has already linked an identity, so a misconfigured provider cannot lock every administrator out of the settings that would fix it. If the organization requires two-factor authentication, including a requirement scheduled for a later date, that administrator also needs two-factor authentication enabled. The check runs again on every save of the enforcement settings while the mode is required, so changing only the session lifetime or the personal key setting is refused too once no administrator qualifies. Once required mode is in effect, the members tab also refuses an enforcement or exemption change that would leave no qualifying administrator.
...
* **Assigning a person** whose Hexpm account has a verified email matching the SCIM `userName` adds them as a member with the role you chose, taking a seat. They're emailed that they were added. If the seats are full, the create is refused or a seat is added to the subscription, per your choice. If the organization requires two-factor authentication and the person has not enabled it, the create is refused until they do; your provider retries on its own schedule.
* **Assigning a current member** keeps their membership and role as they are and takes no seat. A current member is matched by a verified email, or by the address their linked SSO identity last reported, which finds a member whose Hexpm account doesn't hold the work address. Someone who isn't a member is never matched through an SSO identity.
* **Assigning an address no Hexpm account has verified** sends a pending invitation to that address instead. Nothing creates a Hexpm account, and the seat is spent when the invitation is accepted, not when it is sent. An invitation is refused at acceptance if the organization is full by then, whichever seat policy you chose, because acceptance is what spends the seat.
...
With Hex v2.6.0 or later, when a CLI session's organization access lapses, the next `mix deps.get` asks:

```
acme: SSO authentication required. Authenticate now? [Yn]
```
...
* **Block** turns members' personal keys away from this organization. From the moment required mode takes effect, on the required-by date or at the save when there's no date, a request made with a personal key is refused for this organization. The permissions naming the organization are removed from the keys at the next daily run after that, at 03:45 UTC. Their owners are emailed, and the rest of each key keeps working. Creating a key with a permission that names the organization, or every repository, is refused. A key with only API permissions can still be created, and its requests against the organization are refused. Members publishing by hand run `mix hex.user auth` instead, after `mix hex.organization deauth ORGANIZATION` on any machine where they ran `mix hex.organization auth ORGANIZATION`, because `mix` uses the key that command stored ahead of their sign-in. Automation moves to an organization key.
...
Your provider controls authentication to the SSO application. Organization membership and roles are stored in Hexpm, and your provider changes them only through just-in-time membership and provisioning. Removing a provider assignment removes the member from Hexpm only when provisioning is connected; otherwise, remove the member in Hexpm to revoke organization access.
