...
<dl class="dl-horizontal">
  ...
  <dt><code>:secret_scan</code></dt>
  <dd>Configuration for secret scanning (see <a href="#secret-scanning">Secret scanning</a> below). Supports <code>:ignore</code>, a list of file path globs whose findings are suppressed.</dd>
</dl>
...
Published packages are scanned for leaked credentials such as cloud keys, API tokens, and private keys. When a scan finds one, the package owners and the publisher are emailed so they can rotate it. Nothing else happens: the release is not blocked, revoked, or listed anywhere public, and Hex stores only a fingerprint and a masked preview of the value, never the value itself.
...
`:ignore` takes file path globs where `*` matches within a path segment, `**` spans separators, and `?` matches one non-separator character.
