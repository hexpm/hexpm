defmodule HexpmWeb.Blog.Posts do
  ...
  @template_dir @phoenix_root
  @template_hash all_templates |> :erlang.term_to_binary() |> :erlang.md5()

  def __mix_recompile__? do
    Phoenix.Template.find_all(@template_dir)
    |> Enum.map(&Phoenix.View.template_path_to_name(&1, @template_dir))
    |> Enum.flat_map(fn
      <<n1, n2, n3, "-", slug::binary>> = template
      when n1 in ?0..?9 and n2 in ?0..?9 and n3 in ?0..?9 ->
        [{Path.rootname(slug), template}]
