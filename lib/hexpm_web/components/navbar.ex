defmodule HexpmWeb.Components.Navbar do
  ...
  defp desktop_nav(assigns) do
    ...
    <div class="hidden lg:flex items-center flex-1 justify-end gap-10">
      <.search_form :if={@show_search} conn={@conn} search={@search} autofocus={@autofocus_search} />
      <.nav_links />
      <.theme_toggle />
  ...
  defp search_form(assigns) do
    ~H"""
    <div class="min-w-0 flex-1 max-w-[420px] mr-auto">
      {Phoenix.Component.live_render(@conn, HexpmWeb.SearchSuggestionsLive,
        id: "nav-search",
        session: %{"variant" => "nav", "limit" => 8, "autofocus" => @autofocus, "search" => @search}
      )}
    </div>
    """
  end
